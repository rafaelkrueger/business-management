...
const Config: React.FC<{
  ...
  // Estado para dados de formulários existentes
  ...
  // Opções e módulos para o estabelecimento (exemplo)
  ...
  const establishmentModules = {
    ...
    // ... Outras configurações por estabelecimento
  };
  ...
  // Estado e handler para o feedback
  ...
  const handleFeedbackSubmit = (e) => {
    e.preventDefault();
      AllInOneApi.post('/user/contact', {
        ...
        enqueueSnackbar(
          t("feedback.feedbackSentError") || "Error while sending feedback!",
          { variant: "error" }
        );
      })
  };
  ...
  const handleEstablishmentChange = (event, newValue) => {
    ...
    if (translatedValue) {
      ...
      const filteredModules = availableModules
        .filter((module) =>
          selectedModules.includes(t(`config.establishmentModules.${module.key}`))
        )
        .map((module) => ({
          ...module,
          isActive: true,
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      formData.append(
        "name",
        companyData.name || companyInformation?.name
      );
      formData.append(
        "email",
        companyData.email || companyInformation?.email
      );
      formData.append(
        "phone",
        companyData.phone || companyInformation?.phone
      );
      formData.append(
        "document",
        companyData.document || companyInformation?.document
      );
      ...
      enqueueSnackbar(
        t("config.form.companyUpdatedSuccess"),
        { variant: "success" }
      );
    } catch (error) {
      enqueueSnackbar(
        t("config.form.companyUpdateError"),
        { variant: "error" }
      );
    }
  };
                      ...
                      <Button
                        ...
                        onClick={() => {
                          if (originalCompanyData) {
                            ...
                            setSelectedEstablishment(null);
                            enqueueSnackbar(
                              t("notification.canceled-modules"),
                              { variant: "info" }
                            );
                          }
                        }}
                ...
                <Stack spacing={2} flex={1} sx={{ width: "100%" }}>
                  ...
                  >
                    {availableModules?.map((module) => {
                      const isActive = activeModules?.some(
                        (activeModule) =>
                      ...
                      const isChecked =
                        companyData.modules[module.key] ?? isActive ?? false;
                      const translatedModuleName = t(
                        `config.establishmentModules.${module.key}`,
                        module.name
                      );
                      return (
                        <FormControlLabel
                          ...
                          control={
                            <Checkbox
                              checked={isChecked}
                              onChange={(e) =>
                                handleCompanyChange(e, module.key)
                              }
                              name={module.key}
                            />
                          }
                  ...
                  >
                    <Button
                      ...
                      onClick={() => {
                        const modulesPayload = availableModules?.map(
                          (module) => ({
                            id: module.id,
                            key: module.key,
                            name: module.name,
                            isActive:
                              companyData.modules[module.key] ?? false,
                        ...
                        ModulesService.patch(modulesPayload)
                          .then(() => {
                            enqueueSnackbar(
                              t("notification.updated-modules"),
                              { variant: "success" }
                            );
                            props.setModulesUpdating(
                              !props.modulesUpdating
                            );
                          })
                          .catch((error) => {
                            console.error(
          ...
          <CardContent>
            <form onSubmit={handleFeedbackSubmit}>
              <Stack spacing={2}>
                ...
                <TextField
                  label={t("feedback.message") || "Sua mensagem"}
